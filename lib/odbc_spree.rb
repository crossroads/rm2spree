  ...
  module ODBC
    class RM
      ...
      def write_yaml_to_file(filename, data_to_write)
        ...
        File.copy(filename, "#{filename}.backup") if File.exists?(filename)
...
and corresponding products might need to be updated.",
      ...
      def get_product_data(stock_id, stock_records)
        ...
        # On hand should be NET quantity (quantity stock is already minus stock on layby)
        product_data["on_hand"] = stock_records[stock_id]["quantity"].to_i
